/**
 * 대한민국 행정구역 시군구 시드 데이터 (총 229개 시군구 + 17개 시도)
 * 출처: 행정안전부 행정구역 기준. 향후 변경 시 이 파일만 수정하면 됩니다.
 */
...
export interface ProvinceSeed {
  ...
  /** 시도 코드 (정렬/식별용) */
  code: string;
  /** 시군구 목록. 세종처럼 하위 구역이 없으면 빈 배열 */
  districts: string[];
}

export const KOREA_REGIONS: ProvinceSeed[] = [
  {
    province: '서울특별시',
    code: '11',
    districts: [
...
/** 전체 시군구 개수 (대시보드/통계 검증용) */
export const TOTAL_DISTRICT_COUNT = KOREA_REGIONS.reduce(
