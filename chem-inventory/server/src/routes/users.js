...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!['user', 'admin'].includes(role)) throw badRequest('역할은 user 또는 admin이어야 합니다.');
    const updated = await mutate('users', (rows) => {
  ...
  asyncHandler(async (req, res) => {
    const updated = await mutate('users', (rows) => {
      ...
      if (req.body.role !== undefined) {
        ...
        if (!['user', 'admin'].includes(role)) throw badRequest('역할 값이 올바르지 않습니다.');
        // 마지막 관리자 강등 방지
